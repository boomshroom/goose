...
func loadTable(){
	...
	Table[3] = segment.CodeDataDesc{Code: false, User: true}.Pack()
	Table[4], Table[5] = segment.SystemDesc{Base:uint64(uintptr(unsafe.Pointer(&tss[0]))), Limit:uint32(unsafe.Sizeof(tss))-1, Type: segment.TSSAvail}.Pack().Decompose()
}